...
## Control scene
should not contain data for cleaned data, just interesting to see for excluded data

```{r}
data.control_selected = data.test %>% 
  dplyr::select(prolific_id, response, id, type) %>% 
  mutate(control_selected = str_detect(response, "contrast")) %>% 
  group_by(prolific_id, type) %>% 
  summarize(n_control = sum(control_selected), .groups = "drop_last") %>% 
  mutate(N_control = sum(n_control))

data.control_selected

data.control_selected %>% distinct_at(vars(c(prolific_id, N_control))) %>% 
  arrange(N_control)

data.control_selected %>% filter(N_control == 1 & n_control == 1)
```
...
ppc_bars_grouped(y = df.ordinal$response, yrep = yrep, group = df.ordinal$QUD)
